...
parser.add_argument('--exp', type=str,
                    default='ACDC/MLNet', help='experiment_name')
...
def train(args, snapshot_path):
    ...
    optimizer_T = optim.SGD(model.parameters(), lr=base_lr,
                          momentum=0.9, weight_decay=0.0001)
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            ema_inputs = labeled_volume_batch * img_mask + unlabeled_volume_batch * (1 - img_mask)

            outputs = model(volume_batch)
            outputs_soft = torch.softmax(outputs, dim=1)
            a = outputs_soft[:args.labeled_bs] * img_mask + outputs_soft[args.labeled_bs:] * (1 - img_mask)
            a_label=torch.max(a,1)[1]
            ...
            if iter_num < 1000:
                ...
                consistency_loss = torch.mean(
                    (a-ema_output_soft)**2)
                unsupervised_loss = 0.5*(ce_loss(outputs[args.labeled_bs:], ema_output_label.long())+ce_loss(ema_output, a_label.long()))
